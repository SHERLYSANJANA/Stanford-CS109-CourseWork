...
def diff_sample_means(filepath):
    ...
    for x in sample:
        if (x[1]=='activity1') :
            sample_activity1.append(int(x[2]))
        else :
            sample_activity2.append(int(x[2]))
            
    return abs(np.mean(sample_activity1)-np.mean(sample_activity2))
...
def estimate_p_val(filepath, seed=109):
    ...
    len_activity1=0
    len_activity2=0
    observed=diff_sample_means(filepath)
    for x in sample:
        all_scores.append(int(x[2]))
        if(x[1]=='activity1') :
            len_activity1+=1
        else:
            len_activity2+=1
    ...
    for _ in range(10000) :
        group1=np.random.choice(all_scores,size=len_activity1,replace=True)
        group2=np.random.choice(all_scores,size=len_activity2,replace=True)
